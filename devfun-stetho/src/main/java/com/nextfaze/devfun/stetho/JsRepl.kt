...
internal fun generateFunctionName(category: CategoryItem, function: FunctionItem) = "${category.name}_${function.name}".replace(LEGAL_CHARACTERS, "_")

fun buildJsRuntime(context: Context, activityProvider: ActivityProvider, devFun: DevFun): RuntimeReplFactory = TrackingJsRuntimeReplFactoryBuilder(context).apply {
    ...
    addFunction("toast", jsArgsFunction {
        ...
        val str = arg as? CharSequence ?: return@jsArgsFunction JsContext.reportRuntimeError("String argument expected by got ${if (arg != null) arg::class.toString() else "null"}=${it.getOrNull(0)}")
...
private inline fun jsFunction(invokeOnMain: Boolean = true, crossinline body: (cx: JsContext, scope: Scriptable, thisObj: Scriptable, args: Array<out Any>) -> Any) = JsBaseFunction({ cx, scope, thisObj, args -> body.invoke(cx, scope, thisObj, args) }, invokeOnMain)

private class JsBaseFunction(inline val body: (cx: JsContext, scope: Scriptable, thisObj: Scriptable, args: Array<out Any>) -> Any, val invokeOnMain: Boolean = true) : BaseFunction() {
 ...
 */
private fun <T> List<T>.joiner(separator: CharSequence = ", ",
                               prefix: CharSequence = "",
